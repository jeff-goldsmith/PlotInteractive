#' Create input calls for plot_interactive.fosr()
#' 
#' Internal method that constructs the input calls for plot_interactive.fosr(). The
...
createInputCall = function(name, variable){
  ...
  if(is.numeric(variable)){
    ...
    call("sliderInput", inputId = name, label = name, 
         min = signif(min(variable), 2), max = signif(max(variable), 2), value = mean(variable), animate = animationOptions(interval=500))
